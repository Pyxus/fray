...
signal children_changed(node, change)
...
func _on_SceneTree_node_added(node: Node) -> void:
	if node.get_parent() == _parent:
		emit_signal("children_changed", node, Change.ADDED)
...
func _on_SceneTree_node_removed(node: Node) -> void:
	if node.get_parent() == _parent:
		emit_signal("children_changed", node, Change.REMOVED)
...
func _on_ChildNode_script_changed(node: Node) -> void:
	emit_signal("children_changed", node, Change.SCRIPT_CHANGED)
