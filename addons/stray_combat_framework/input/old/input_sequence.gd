...
func is_valid_input(inputs: Array) -> bool:
	...
	for i in len(inputs):
		# So here we can can check if the sequence input's input is a Contextual Input
		# Then if it is we fetch the input based on the context and use that instead of the sequence_input.input
