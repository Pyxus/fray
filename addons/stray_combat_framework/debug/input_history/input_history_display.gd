...
func add_input(input: DetectedInput) -> void:
    if abs(_last_input_time_stamp - input.time_stamp) / 1000.0 >= .5:
    ...
    input_view.input_id = input.id
    input_view.time_stamp = input.time_stamp
    input_view.id_label_visible = input_id_visible

    _last_input_time_stamp = input.time_stamp

    if _texture_by_id.has(input.id):
        input_view.icon_texture = _texture_by_id[input.id]
